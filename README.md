...
- 每 30 分钟执行一次 R1/R3/R4（每小时第 5 分和第 35 分）；
- 每天 13:30（Asia/Shanghai）执行 R2；
- 周末照常运行；
- 服务器重启后自动恢复，错过的任务会补跑；
- 原有文件锁继续阻止两个任务并发执行。
- 单次任务最多运行 28 分钟，下载重试预算 25 分钟，两者都卡在 30 分钟调度间隔以内；
- 锁冲突按正常跳过处理：上一轮没跑完时下一轮以退出码 3 跳过，并在日志里留一条 error。
  13:30 的 R2 如果跑满预算，会盖掉 13:35 那一次 R1/R3/R4，14:05 恢复正常，属预期行为。
  正式发送启用后，任务失败会向同一飞书群发送告警。
