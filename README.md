...
- **预设触发器只按结构定位，绝不按它显示的文字**。它显示的是「上一次用过的预设」——谁在这个账号上切过就变成谁的那个，是个变量。选择器用 `.el-popover__reference` 类名和里面那个 `.show-search-list` 箭头图标，两条都跟文字无关。下拉已经开着时不再点它，那一下会把它关掉。
- **列表项只在可见的 `.search-item` 里按整词比对**。列表里有「正向」和「上海正向」这种互为子串的名字，子串匹配会选错；Element UI 还会在 DOM 里留下隐藏的 popover 副本，点它等于什么都没点。
- **选完必须回读触发器核对**。这是唯一能证明「这一下真的选中了」的信号：2026-08-17 17:54 那轮就是没选中却一路跑到底，带着继承来的 12644 行视图（正常 4750）导出、算规则、发飞书，全程没有任何一处报错。核对不过直接中止本轮；标题读不到则只记警告，不凭空造一个失败点。
