...
- 使用 Playwright 登录李宁 TMS、进入集团订单管理、应用保存筛选、创建导出任务，按任务 id 认领本轮产物并从下载中心取得完整 Excel；
...
李宁 TMS 的导出是后台任务，`TmsDownloader` 按这个顺序跑完一轮：

1. 全新浏览器登录（不复用 profile——TMS 是标签页式 SPA，上一轮的 DOM 状态会带过来）；
2. **先进下载中心，记下现有导出链接里最大的任务 id**，作为本轮基线；
3. 打开集团订单管理、选回预设视图、查询，等分页读出真实条数（读不到就直接失败，不在空表上点导出）；
4. 点导出并确认；
5. 回下载中心轮询，直到出现 id 大于基线的导出链接——那就是本轮的产物；
6. 用浏览器会话直接 GET 这个链接，按 magic bytes 确认真是 xlsx/xls 再落盘。

第 2 步和第 5 步是配套的：任务 id 自增，「id 比基线大」是判断某个文件属不属于本轮的唯一可靠依据。第 6 步的格式校验挡住的是 TMS 会话过期时返回 HTML 却仍带 200 状态码的情况——不校验的话这个 HTML 会被存成 `.xls`，然后在解析阶段报出跟真实原因毫无关系的错。

页面元素全部由 `tms.selectors` 配置，凡是会重复渲染的控件（工具栏、分页、菜单项）选择器都带 `:visible`，由选择器引擎过滤掉隐藏的标签页副本。首次拿到账号后需要用有界面模式校准一次：
