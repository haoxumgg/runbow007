...
# Playwright 的浏览器二进制默认走上游 cdn.playwright.dev。阿里云不镜像它，而
# npmmirror 对 1.62.0 锁定的 chromium revision 1234 实测返回 404，所以这里不设
# 默认镜像——宁可慢，也不给一个会 404 的默认值。确认某个镜像站有对应 revision
# 后，用 PLAYWRIGHT_MIRROR=<host> 覆盖即可（会被设成 PLAYWRIGHT_DOWNLOAD_HOST）。
